...
func setVMOptions(conf *VirtualMachine) *vsphere.VirtualMachineArgs {
	var diskArray vsphere.VirtualMachineDiskArray
	var networkArray vsphere.VirtualMachineNetworkInterfaceArray
	if conf.Disks != nil {
...
func setResourceIDS(ctx *pulumi.Context, conf *VirtualMachine) error {
...
func (v *VirtualMachine) BuildVMRunFunc(vm *VirtualMachine) pulumi.RunFunc {
	return func(ctx *pulumi.Context) error {
		...
		if vm.Template != "" {
			...
			cloneArgs = newCloneArgs(templateClone.Uuid)
		}

		if vm.Count > 0 {
			// Pre-Compile list of vm names
			var vmNames []string
			for i := 0; i < vm.Count; i ++ {
				...
				vmNames = append(vmNames, fmt.Sprintf("%s%s", vm.Name, suffix))
			}
			...
			for _, name := range vmNames {
				...
				vmResult, err := vsphere.NewVirtualMachine(ctx, name, vmArgs)
				if err != nil {
					return err
				}
